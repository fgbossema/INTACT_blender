...
def CuttingCurveAdd():
    ...
    CurveCutter.name = "INTACT_CuttingCurve_1"
    curve = CurveCutter.data
    curve.name = "INTACT_CuttingCurve_1_Mesh"
    bpy.context.scene.INTACT_Props.CurveCutterNameProp = CurveCutter.name
...
def VertexPaintCut(mode):
    ...
    for vg in ActiveObj.vertex_groups:
        if "INTACT_PaintCutter_" in vg.name:
            ActiveObj.vertex_groups.remove(vg)

    Area_vg = ActiveObj.vertex_groups.new(name="INTACT_PaintCutter_Area_vg")
    ActiveObj.vertex_groups.active_index = Area_vg.index
    bpy.ops.object.vertex_group_assign()
    bpy.ops.mesh.region_to_loop()
    Border_vg = ActiveObj.vertex_groups.new(name="INTACT_PaintCutter_Border_vg")
    bpy.ops.object.vertex_group_assign()
    ...
    if mode == "Cut":
        ...
        bpy.ops.intact.separate_objects(SeparateMode="Selection")
    ...
    if mode == "Make Copy (Shell)":
        ...
        shell.name = "Shell"
        # Add color material :
        mat = bpy.data.materials.get("INTACT_PaintCut_mat") or bpy.data.materials.new(
            "INTACT_PaintCut_mat"
        )
